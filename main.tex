...
\node[groupbox, draw, fill=white, align=left] at (5*x, 18.5*y) {
    $\begin{aligned}
...
\node[tag] (t9) at (0*x, 19*y) {$*$\,其他门清限定};
    \node[yi] (ph) at (x, 19*y) {$*$\,平和};
    \node[yi] (lz) at (x, 20*y) {$*$\,立直};
    \node[yi] (qdz) at (2*x, 19*y) {$*$\,七对子};
    \draw[arr, dashed, to path={.. controls (3.9*x,18*y) and (3.9*x,10*y) .. (\tikztotarget) \tikztonodes}] (qdz) to node[near end, above ,sloped] {全是幺} (hlt);

\node[tag] (t10) at (0, 22*y) {看运气};
    \node[yi] (yf) at (x, 22*y) {$*$\,一发};
    \draw[arr] (lz) -- node[right] {1巡内} (yf);
    \node[yi] (qg) at (x, 24*y) {抢杠};
    \node[yi] (lskh) at (x, 25*y) {岭上开花};
    \node[yi] (mqqzmh) at (x, 23*y) {$*$\,门前清自摸和};
    \node[tag] (t11) at (.3*x, 26.3*y) {和最后1张};
    \node[yi] (hdly) at (x, 26*y) {海底捞月};
    \node[yi] (hdmy) at (x, 27*y) {河底摸鱼};
    \draw[arr] (t11) to node[near end, above] {自摸} (hdly);
    \draw[arr] (t11) to node[near end, below] {荣和} (hdmy);

    \node[yi] (llz) at (2*x, 22*y) {$*$\,两立直};
    ...
    \node[tag] (t12) at (6.3*x, 25.3*y) {和第1张};
...
\node[groupbox, fit=(jlbd)(lys)(4ak)(sakdq)(4gz)(qlt)(dsy)(xsx)(dsx)(gsws)(th)(dh)(zys)(7f), fill=purple!10] {};

\node[groupbox, draw = gray, rounded corners=10pt, dashed, fit=(t10)(t11)(yf)(mqqzmh)(gsws)(gswsssm)(qg)(hdly)(hdmy)(th)(dh)] {};
\node[groupbox, draw = gray, rounded corners=10pt, dashed, fit=(t9)(ph)(lz)(qdz)] {};

% \node[tag] (t13) at (0, 27*y) {不能和};
...
\begin{center}
    \textcolor{gray}{
    \scriptsize \ttfamily
    基于：wiki.lingshangkaihua.com\quad 字体：mahjongfont.pages.dev}
\end{center}
