...
v1.0.3\quad 版本日期：\today\\
本图由初学者整理。如发现问题，敬请反馈至GitHub仓库：github.com/ErSanSan233/riichi-mahjong-one-page\\~
}
\end{center}
...
\node[groupbox, draw=magenta!80!black, 
align=left, text=magenta!80!black] at (5*x, 19.7*y) {
    $\begin{aligned}
        &\text{幺九牌(幺)}\left\{\begin{aligned}
            &\text{老头牌：{\mj 1s 9s 1p 9p 1m 9m}}\\
            &\text{字牌}\left\{\begin{aligned}
                &\text{风牌：{\mj 1z 2z 3z 4z}}\\
                &\text{三元牌：{\mj 5z 6z 7z}}
            \end{aligned}\right. 
        \end{aligned}\right. \\
        &\text{绿牌：{\mj 2s 3s 4s 6s 8s 6z}}\\
        &\text{带有} * \text{的役种为门清限定}
    \end{aligned}$
};
...
\node[tag] (t1) at (0, .3*y-y) {特殊顺子};
    ...
    \draw[arr] (t1) to node[near end, above] {3色同数字} (ssts);
    ...
    \draw[arr] (t1) to node[near end, above] {从1到9} (yqtg);
    ...
    \draw[arr] (ybk) -- node[above] {再来1对} (ebk);
...
\node[tag] (t4) at (0.5*x,5.3*y) {3组刻杠};
    ...
    \node[yi] (zys) at (7*x,10*y) {字一色};
    \node[circle, fill=black, minimum size=4pt, inner sep=0pt] (x2) at (6*x,9*y) {};
    \draw[arr] (t4) to node[near end, above] {全是幺} (hlt);
    \draw (hlt) -- node {} (x2);
    \draw[arr] (x2) -- node[above] {没有字牌} (qlt);
    \draw[arr] (x2) to node[near end, below] {全是字牌} (zys);

\node[tag] (t5) at (0,9.3*y) {幺};
\node[tag] (t6) at (0.5*x,10*y) {老头};
    \draw[arr] (t5) to (t6);
    \draw[arr] (t6) to[to path={-| (\tikztotarget) \tikztonodes}] node[near start, above] {7对幺} (hlt);
    ...
    \node[tag] (t7) at (.5*x, 14*y+y+y) {三元};
    \node[yi] (xsy) at (2*x, 14*y+y+y) {小三元};
    \node[yi] (dsy) at (7*x, 14*y+y+y) {大三元};
    \draw[arr] (t7) -- node[above] {{\mj 5z5z5z6z6z6z7z7z7z}凑8张} (xsy);
    \draw[arr] (xsy) -- node[near end, above] {凑齐} (dsy);
    \draw[arr] (t5) to (t7);

    \node[tag] (t8) at (.5*x, 15*y) {四喜(风)};
    \node[yi] (xsx) at (7*x, 15*y) {小四喜};
    \node[yi] (dsx) at (7*x, 13*y) {大四喜};
    \draw[arr] (t8) -- node[near end, above] {{\mj 1z1z1z2z2z2z3z3z3z4z4z4z}凑11张} (xsx);
    \draw[arr] (xsx) -- node[right] {凑齐} (dsx);
    \draw[arr] (t5) to (t8);

    \node[yi] (gsws) at (7*x, 17*y) {$*$\,国士无双};
    \node[yi] (gswsssm) at (7*x, 19*y) {$*$\,国士无双十三面};
    \draw[arr] (t5) to node[near end, above, xshift=6cm] {凑齐所有幺各1张再加其中任1张} (gsws);
    \draw[arr] (gsws) -- node[right] {听13张} (gswsssm);

    \node[yi] (dyj) at (x, 16*y+y+y) {断幺九};
    \draw[arr] (t5) to node[near end, above] {没有幺} (dyj);

    \node[yi] (y) at (x, 17*y+y+y) {役牌};
    \draw[arr] (t5) to node[near end, above] {自风/场风/三元} (y);

    \node[yi] (ljmg) at (4*x, 14*y) {流局满贯};
    \draw[arr] (t5) to node[near end, above] {只打幺，不被吃碰杠，直到流局} (ljmg);

\node[tag] (t9) at (0*x, 19*y+y) {$*$\,其他门清限定};
    \node[yi] (ph) at (x, 19*y+y) {$*$\,平和};
    \node[yi] (lz) at (x, 20*y+y) {$*$\,立直};
    \node[yi] (qdz) at (2*x, 19*y+y) {$*$\,七对子};
    \draw[arr, dashed, to path={.. controls (3.9*x,18*y+y) and (3.9*x,10*y) .. (\tikztotarget) \tikztonodes}] (qdz) to node[near end, above ,sloped] {全是幺} (hlt);

\node[tag] (t10) at (0, 22*y+y) {看运气};
    \node[yi] (yf) at (x, 22*y+y) {$*$\,一发};
    \draw[arr] (lz) -- node[right] {1巡内} (yf);
    \node[yi] (qg) at (x, 24*y+y) {抢杠};
    \node[yi] (lskh) at (x, 25*y+y) {岭上开花};
    \node[yi] (mqqzmh) at (x, 23*y+y) {$*$\,门前清自摸和};
    \node[tag] (t11) at (.3*x, 26.3*y+y) {和最后1张};
    \node[yi] (hdly) at (x, 26*y+y) {海底捞月};
    \node[yi] (hdmy) at (x, 27*y+y) {河底摸鱼};
    \draw[arr] (t11) to node[near end, above] {自摸} (hdly);
    \draw[arr] (t11) to node[near end, below] {荣和} (hdmy);

    \node[yi] (llz) at (2*x, 22*y+y) {$*$\,两立直};
    \draw[arr] (lz) to[to path={-| (\tikztotarget) \tikztonodes}] node[near end, right] {第1次舍牌} (llz);

    \node[yi] (th) at (7*x, 23*y) {$*$\,天和};
    \node[yi] (dh) at (7*x, 24*y) {$*$\,地和};
    \node[tag] (t12) at (6.3*x, 23.3*y) {和第1张};
    \draw[arr] (t12) to node[near end, above] {庄家} (th);
    \draw[arr] (t12) to node[near end, below] {闲家} (dh);
...
\begin{pgfonlayer}{background}% 6字及以上的胶囊没有纳入，因为会撑宽方块，影响美观。
\node[groupbox, fit=(yqtg)(ssts)(ybk)(hqdyj)(dyj)(y)(ph)(lz)(yf)(qg)(hdly)(hdmy)(1f), fill=red!10] {};
\node[groupbox, fit=(*yqtg)(*ssts)(hys)(ddh)(sstk)(3ak)(3gz)(hlt)(*hqdyj)(cqdyj)(xsy)(qdz)(llz)(2f), fill=orange!10] {};
...
\node[groupbox, fit=(jlbd)(lys)(4ak)(sakdq)(4gz)(qlt)(dsy)(xsx)(dsx)(gsws)(th)(dh)(zys)(7f), fill=violet!10] {};

\node[groupbox, draw = gray, rounded corners=10pt, dashed, fit=(t10)(t11)(yf)(mqqzmh)(qg)(hdly)(hdmy)(th)(dh)] {};
\node[groupbox, draw = gray, rounded corners=10pt, dashed, fit=(t9)(ph)(lz)(qdz)] {};
...
\begin{center}
    \textcolor{gray}{
    \scriptsize \ttfamily
    基于：wiki.lingshangkaihua.com $\cdot$ 
    字体：mahjongfont.pages.dev $\cdot$
    \href{https://github.com/ErSanSan233/IIIA}
    {\includegraphics[height=10pt, valign=c]{img/iiia-1.pdf}} $\cdot$ 
    \href{https://www.bilibili.com/opus/1173752783218671623}
    {\includegraphics[height=8pt, valign=c]{img/logo.pdf}}
    }
\end{center}
