...
\node[tag] (t6) at (0.5*x,10*y) {老头};
    ...
    \node[yi] (ysy) at (7*x, 14*y) {大三元};
    \draw[arr] (t7) -- node[above] {{\mj 5z5z5z6z6z6z7z7z7z}凑8张} (xsy);
    \draw[arr] (xsy) -- node[above] {凑齐} (ysy);
    \draw[arr] (t5) to (t7);
...
% \node[yi] (y2) at (x,-2*y) {1};
\node[yi] (y3) at (2*x,-2*y) {2};
\node[yi] (y4) at (3*x,-2*y) {3};
\node[yi] (y5) at (4*x,-2*y) {满贯};
\node[yi] (y6) at (5*x,-2*y) {5};
\node[yi] (y7) at (6*x,-2*y) {6};
\node[yi] (y8) at (7*x,-2*y) {役满};

\begin{pgfonlayer}{background}
\node[groupbox, fit=(yqtg)(ssts)(ybk)(hqdyj)(dyj)(y)(ph)(lz)(yf)(mqqzmh)(qg)(hdly)(hdmy)] (1f) {};
\node[above=2pt of 1f, font=\small\bfseries] {1番} ;



\end{pgfonlayer}

% \node[groupbox, fit=(y1)(y2)(y3)] (g1) {};
% \node[above=2pt of g1, font=\small\bfseries] {门清限定};

% \node[groupbox, fit=(y4)(y5)(y6)(y7)] (g2) {};
% \node[above=2pt of g2, font=\small\bfseries] {一般型};

% \draw[arr] (g1) -- node[midway,above] {可叠加} (g2);
\end{tikzpicture}
